(function () {
  ...
  function renderConvList() {
    ...
    conversations.forEach(conv => {
      ...
      btnEdit.title = "修改标题";
      btnEdit.setAttribute("aria-label", "修改标题");
      btnEdit.textContent = "✎";
      ...
      btnDel.title = "删除此会话";
      btnDel.setAttribute("aria-label", "删除此会话");
      btnDel.textContent = "×";
  ...
  function ensureAssistantMessage() {
    ...
    if (nodes.length) {
      return { conv, wrap: nodes[nodes.length - 1].parentElement, bubble: nodes[nodes.length - 1] };
    }
